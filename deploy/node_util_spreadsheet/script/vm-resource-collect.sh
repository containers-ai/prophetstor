...
DEF_LOG_FILE="/var/log/vm-resource-collect.log"
...
function aws_clusters()
{
    ...
    do
        ...
        if [ "${cluster_name}" != "" -a "${cluster_type}" = "vm" -a "${data_source}" = "aws" -a "${status_active}" = "true" ]
    ...
    then
        if [ ${#c_list[@]} -eq 0 ]
        then
            logging "${ERR}" "No AWS VM cluster is configured."
            retcode=1
        else
            logging "AWS VM clusters: ${c_list[@]}"
        fi
    fi
    return ${retcode}
}
...
function vm_info()
{
    ...
    do
        case "${k}" in
            ${API_ERROR_KEY})
                logging "${ERR}" "Federator.ai resources/aws/vm API (${vmname}): ${v}"
                retcode=1
                break ;;
            data\.*\.cpu_cores)
                cpu_cores=${v} ;;
...
function vm_predictions()
{
    ...
    do
        ...
        case "${granularity}" in
            ...
            *)
                logging "${ERR}" "Unknown prediction granualrity: ${granularity}"
                retcode=1
                break ;;
        esac
    ...
    if [ ${retcode} -eq 0 ]
    then
        logging "PREDITION: VM ${vmcluster}/${vmname}: ${pred_mcores},${pred_mbytes}"
    fi
    return ${retcode}
}
...
function create_vm_csv()
{
    ...
    touch ${vars[csv_dir]}/${VM_IDV_CSV} >/dev/null 2>&1
    touch ${vars[csv_dir]}/${VM_ASG_CSV} >/dev/null 2>&1

    if [ "${vars[target_cluster]}" = "" ]
    then
        # get the list of AWS VM clusters
        if ! aws_clusters cluster_list
        then
            return 1
        fi
    else
    ...
    do
        ...
        do
            # get VM basic info
            if ! vm_info ${vm_name} "${cluster_region}" vm_subtype vm_data_string vm_cores vm_mem_bytes
            then
...
function banner()
{
    banner_string="Federator.ai AWS VM Resource Collector v${VER}"
    echo ${banner_string}
