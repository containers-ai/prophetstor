...
#   1.0.6 - Collect persistent/storage/unattached volume information
#   1.0.7 - Fix failed to collect GKE persistent volumes
#
VER=1.0.7
...
function vm_clusters()
{
    ...
    do
        ...
        if [[ "${cluster_name}" != "" ]] && [[ "${cluster_type}" = "${req_type}" ]] && [[ "${status_active}" = "true" ]]
        then
        ...
        if [[ "${cluster_name}" != "" ]] && [[ "${cluster_type}" != "" ]] && [[ "${data_source}" != "" ]] && [[ "${status_active}" != "" ]]
        then
            cluster_name=""
            data_source=""
            status_active=""
...
function create_vm_csv()
{
    ...
    then
        ...
        if ! vm_clusters "vm" cluster_list
        then
            return 1
        fi
    else
...
function cluster_volumes()
{
    ...
    local vol_capacity=""
    local vol_iops=""
    local vol_throughput=""
    local vol_active=""
    local retcode=0
    local count=0

    url="${HTTPS}://${vars[f8ai_host]}/apis/v1/resources/clusters/${cluster}/${ctype}"
    ...
    do
        ...
        case "${k}" in
            ${API_ERROR_KEY})
                logging "${ERR}" "Federator.ai Volume API (${cluster}/${ctype}): ${v}"
                retcode=1
                break ;;
            data\.*\.pv_name)
                vol_name=${v} ;;            
            ...
            data\.*\.desired_iops)
                vol_iops=${v} ;;            
            data\.*\.desired_throughput)
                vol_throughput=${v} ;;            
            data\.*\.active)
                vol_active=${v} ;;            
        esac

        if [[ "${vol_name}" != "" ]] && [[ "${vol_capacity}" != "" ]] && [[ "${vol_active}" != "" ]]
        then
            if [ "${vol_active}" = "true" ]
            then
                echo "${cluster},${ctype},${vol_name},${vol_id},${vol_type},${vol_region},${vol_capacity},${vol_iops},${vol_throughput}" >> "${vars[csv_dir]}/${VOLUME_CSV}"
                count=$((count + 1))
            fi
            vol_name=""
            vol_type=""
    ...
    if [ ${retcode} -eq 0 ]
    then
        logging "Cluster ${cluster}: ${count} ${ctype}"
    fi
    return ${retcode}
}

function create_volume_csv()
{
    logging "${STDOUT}" "Start collecting volume data."

    rm -rf "${vars[csv_dir]:?}/${VOLUME_CSV}" >/dev/null 2>&1

    echo "${VER},${NOW},${vars[target_cluster]},${vars[f8ai_granularity]}" >> "${vars[csv_dir]}/${VOLUME_CSV}"

    if [ "${vars[target_cluster]}" = "" ]
    then
        # get the list of Kubernetes clusters
        if ! vm_clusters "k8s" k8s_cluster_list
        then
            return 1
        fi
        # get the list of VM clusters
        if ! vm_clusters "vm" vm_cluster_list
        then
            return 1
        fi
    else
        k8s_cluster_list=()
        vm_cluster_list=( "${vars[target_cluster]}" )
    fi

    for cluster_name in "${k8s_cluster_list[@]}"
    do
        # get volumes in the cluster
        if ! cluster_volumes "${cluster_name}" "persistentvolumes"
        then
            logging "${WARN}" "Failed to collect persistent volume inforamtion for Kubernetes cluster ${cluster_name}"
            # continue
        fi
        echo -n "."
    done

    for cluster_name in "${vm_cluster_list[@]}" "aws_unattached_volume_cluster"
    do
        # get volumes in the cluster
        if ! cluster_volumes "${cluster_name}" "storagevolumes"
        then
            logging "${WARN}" "Failed to collect storage volume inforamtion for VM cluster ${cluster_name}"
            # continue
        fi
        echo -n "."
    done

    echo >> "${vars[csv_dir]}/${VOLUME_CSV}"
    echo
}
...
Options:
  ...
  -v, --volume=''         Collect volume data (DEFAULT: 'yes')
...
if [ "${vars[volume]}" = "yes" ]
then
    if ! create_volume_csv
    then
        logging "${STDOUT}" "${ERR}" "Failed to create Volume .csv file."
        exit 1
    else
        logging "${STDOUT}" "${INFO}" "Successfully created Volume .csv: '${vars[csv_dir]}/${VOLUME_CSV}'."
    fi
fi